...
ArtifactoryClient.prototype.getNpmConfig = function (login, password, repoKey, scope) {
  ...
  if (this.logging) {
    console.log('getNpmConfig: sending ' + options.url);
  }
  var that = this;
  return request.get(options).then(function (response) {
    if (response.statusCode === 200) {
      if (that.logging) {
        console.log('getNpmConfig: success: \n' + response.body);
      }
    ...
    if (response.body) {
      try { result = JSON.parse(response.body); } catch(e) {
        if (that.logging) {
          console.log('getNpmConfig: failed to parse error respose: ' + e);
        }  
      }
    }
    return Promise.reject({statusCode: response.statusCode, result: result && result.errors ? result.errors[0] : result });
  });
}
 ...
 * @paran [object] params Optional querystring params
 */
 ...
 * @returns {Promise} A Promise to a encrypted password
 */
ArtifactoryClient.prototype.getEncryptedPassword = function (login, password) {
  ...
  if (this.logging) {
    console.log('getEncryptedPassword: sending ' + options.url);
  }
 ...
 * @returns {Promise} A Promise to a json object with the file's info as specified in the {@link http://www.jfrog.com/confluence/display/RTF/Artifactory+REST+API#ArtifactoryRESTAPI-FileInfo|FileInfo} Artifactory API.
 */
ArtifactoryClient.prototype.getFileInfo = function (repoKey, remotefilePath) {
  ...
  return request.get(this.getRequestOptions(actionPath)).then(function(response) {
 ...
 * @returns {Promise} A Promise to a json object with the folder's info as specified in the {@link http://www.jfrog.com/confluence/display/RTF/Artifactory+REST+API#ArtifactoryRESTAPI-FolderInfo|FolderInfo} Artifactory API.
 */
...
ArtifactoryClient.prototype.isPathExists = function (repoKey, remotefilePath) {
  ...
  return request.head(this.getRequestOptions(actionPath)).then(function (response) {
 ...
 * @returns {Promise} A Promise to a json object with creation info as specified in the {@link http://www.jfrog.com/confluence/display/RTF/Artifactory+REST+API#ArtifactoryRESTAPI-DeployArtifact|DeployArtifact} Artifactory API.
 */
ArtifactoryClient.prototype.uploadFile = function (repoKey, remotefilePath, fileToUploadPath, forceUpload) {
  var overwriteFileInServer = forceUpload || false,
  ...
  if (!isRemote && !fs.existsSync(fileToUpload)) {
    return Promise.reject('The file to upload ' + fileToUpload + ' does not exist');
  }
  ...
  return this.isPathExists(repoKey, remotefilePath).then(function (fileExists) {
    if (fileExists && !overwriteFileInServer) {
      return Promise.reject('File already exists and forceUpload flag was not provided with a TRUE value.');
    }

    var stream = isRemote ? request(fileToUpload) : fs.createReadStream(fileToUpload);
    // In any other case then proceed with *upload*
    stream.pipe(request.put(this.getRequestOptions(actionPath)).then(function(response) {
      //We expect a CREATED return code.
      if (response.statusCode !== 201) {
        return Promise.reject('HTTP Status Code from server was: ' + response.statusCode);
      }
